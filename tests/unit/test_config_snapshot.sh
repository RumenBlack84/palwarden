...
out="$(snapshot "$WORK/snaps" PALWORLD_USER="$ME" PALWORLD_GROUP="$MY_GROUP" 2>"$WORK/err1")"
snap_dir="$out"
...
# Non-root can only chown to itself, which is exactly the assertion that matters:
# the names are *read from the environment* and applied to the whole tree. The
# hardcoded version could not pass this — "palworld" does not exist on CI either.
assert_eq "$(stat -c '%U %G' "$snap_dir")" "$ME $MY_GROUP" "the snapshot dir gets PALWORLD_USER:PALWORLD_GROUP"
bad_owner=0
for f in "$snap_dir"/*; do
  [ "$(stat -c '%U %G' "$f")" = "$ME $MY_GROUP" ] || { bad_owner=1; echo "  (wrong owner on $f)"; }
done
