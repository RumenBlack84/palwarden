...
- [Status](#status)
- [Libraries](#libraries)
- [Third-party binary](#palworld-config-parser)
...
Reads `/etc/palworld/settings.env`, backs up the current
`PalWorldSettings.ini` to `/opt/palworld/config-backups/`, runs
`palworld-config-parser` to apply non-secret keys, then writes
`AdminPassword`/`ServerPassword` directly (kept out of parser logs and Discord
diffs). Regenerates the pretty INI, posts a redacted diff + settings summary,
and records a config event marker. **Requires a service restart to take effect.**

### `palworld-engine-config`
`/usr/local/sbin/palworld-engine-config {apply [--dry-run]|status [--check]|rollback [--list|--dry-run] [BACKUP]}`
...
and (`--check`) flags drift; `rollback` restores a prior backup. Records event
markers. Reminds you to `graceful-restart` — it never restarts on its own.
